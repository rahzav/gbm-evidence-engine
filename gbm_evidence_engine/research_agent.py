...
def configured_model(explicit_model: str | None = None) -> str:
    return (explicit_model or os.getenv("OPENAI_MODEL") or DEFAULT_MODEL).strip()


def has_api_key(explicit_key: str | None = None) -> bool:
    return bool((explicit_key or os.getenv("OPENAI_API_KEY") or "").strip())
...
def run_agent_turn(
    ...
    *,
    ...
    if client is None:
        ...
        if OpenAI is None:
            raise ResearchAgentError("The OpenAI client dependency is unavailable.")
