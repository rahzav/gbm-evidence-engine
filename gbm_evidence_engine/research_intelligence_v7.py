...
def evaluate_gene_pair(gene_a: str, gene_b: str) -> dict:
    base = v6.evaluate_gene_pair(gene_a, gene_b)
    a = build_research_profile(base["gene_a"])
    b = build_research_profile(base["gene_b"])
    state_comp = _state_complementarity(a.live.get("gbmap_cell_state", {}), b.live.get("gbmap_cell_state", {}))
    model_a = a.live.get("model_relevance", {})
    model_b = b.live.get("model_relevance", {})
    ...
    pair_confidence = None if conf_a is None or conf_b is None else (conf_a + conf_b) / 2

    components = dict(base.get("components") or {})
    components["malignant_cell_state_complementarity"] = None if state_comp is None else round(state_comp, 1)
    components["pair_evidence_confidence"] = None if pair_confidence is None else round(pair_confidence, 1)
    base["components"] = components
    base["model_relevance"] = {a.gene: model_a, b.gene: model_b}
    base["cell_state_context"] = {
    ...
    if state_comp is not None:
        if state_comp >= 55:
            base.setdefault("why_test_it", []).append(
                "The targets show meaningfully different normalized malignant-state expression patterns in the GBmap reference, supporting a complementary-state experiment."
            )
        elif state_comp < 25:
            base.setdefault("risks", []).append(
                "The targets show highly overlapping malignant-state expression patterns; apparent combination value may reflect redundant state coverage."
            )
    if any((x.get("level") in {"limited", "low", "unknown"}) for x in (model_a, model_b)):
        base.setdefault("risks", []).append(
            "At least one target is supported mainly by limited model systems; validate the pair in patient-derived/3D GBM models before interpreting combination rationale."
        )
    base["pair_evidence_confidence"] = _confidence(
        pair_confidence,
    ...
    base["caveat"] = (
        "Combination Rationale prioritizes a pair for experimental testing. Cell-state complementarity, network structure and individual target evidence do not establish pharmacologic synergy, efficacy or safety."
    )
    return base
